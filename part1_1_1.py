...
def cross_entropy(M, w, b, l, x, y):
    loss = tf.reduce_sum(tf.nn.sigmoid_cross_entropy_with_logits(tf.reduce_sum(tf.reduce_sum(w * x, 1), 1) + b, tf.transpose(y)), 1) / M
...
def accuracy(M, w, b, x, y):
    ...
    prediction = tf.to_float(tf.greater(tf.reduce_sum(tf.reduce_sum(w * x, 1), 1) + b, 0.5))
